...
def rmseDict(magnitude_error, signed_angular_error, filename, remove_stat_pixels = False, correlation_mask = False):
        
    mean_me = np.mean(magnitude_error)
    std_me = np.std(magnitude_error,ddof=1)
    rmse_me = np.sqrt((magnitude_error ** 2).mean())

    mean_ae = np.mean(signed_angular_error)
    std_ae = np.std(signed_angular_error,ddof=1)
    ...
    dict = {"dataset": fn, 
            ...
            "magnitude_err_stdev": std_me,
            "magnitude_err_rmse": rmse_me,
            "mean_angular_error" : mean_ae,
            "angular_err_stdev": std_ae,

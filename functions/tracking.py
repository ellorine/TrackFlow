...
def interpolateSparseDisplacements(prediction_grid, results_folder, file_name, ref_dsm, filter_on_correlation_coeff = False, scale_to_dsm=True,
                                   scale_measure=None):
    ...
    if filter_on_correlation_coeff == True:
        ...
        prediction_points = prediction_points.drop(prediction_points[(prediction_points.max_corrcoeff < 0.6)].index)
    else:
        prediction_points = prediction_points.drop(prediction_points[(prediction_points.dx ==-9999) & (prediction_points.dy ==-9999)].index)

    prediction_points = prediction_points.reset_index()
    ...
    interpolated_u_grid = griddata(all_points, prediction_points['dx'], (x_grid, y_grid), method="cubic") 
    interpolated_v_grid = griddata(all_points, prediction_points['dy'], (x_grid, y_grid), method="cubic") 
    interpolated_cc_grid = griddata(all_points, prediction_points['max_corrcoeff'], (x_grid, y_grid), method="cubic",)
    
    ## Propogate masked values to all values in 40 m px grid around them 
    mask_points = pd.read_csv(prediction_grid)
    x = mask_points[' X']
    y = mask_points['Y']
    
    if filter_on_correlation_coeff == True:
        val = mask_points['max_corrcoeff']
    else:
        val = mask_points['dx']
    ...
    for i in range(len(x)):
        ...
        all_x_pos.append(x_pos)
        all_y_pos.append(y_pos)
        all_points.append([x_pos,y_pos,val[i]])
        
    no_data_mask = np.zeros([metadatadict.get('ysize'), metadatadict.get('xsize')])
    ...
    for p in all_points:
        ...
        value = p[2]
        if filter_on_correlation_coeff == True:
            if value < 0.6: #if correlation coefficient is less than threshold
                for i in range(20):
                    for j in range(20):
                        if (y_ind + i < len(no_data_mask)) & (x_ind + j < len(no_data_mask[0])) & (y_ind - i >=0) & (x_ind >=0):
                            no_data_mask[y_ind+i][x_ind+j] = 1
                            no_data_mask[y_ind-i-1][x_ind-j-1] = 1
        else:
            if value==-9999: #if nodata
                for i in range(20):
                    for j in range(20):
                        if (y_ind + i < len(no_data_mask)) & (x_ind + j < len(no_data_mask[0])) & (y_ind - i >=0) & (x_ind >=0):
                            no_data_mask[y_ind+i][x_ind+j] = 1
    ...
    no_data_mask = no_data_mask.astype(np.bool)
    
    interpolated_u_grid[no_data_mask]=-9999
    interpolated_v_grid[no_data_mask]=-9999
    interpolated_cc_grid[no_data_mask]=-9999

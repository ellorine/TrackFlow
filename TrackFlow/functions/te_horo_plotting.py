...
def plotMaskedVelocities_1x3(data_list, u_bands, v_bands, titles_list, dsm_list, outfolder):
    ...
    for i, d in enumerate(data_list):
        ...
        im = axes[col_pos].pcolormesh(vel_3d_mm,vmin=0, vmax=200,cmap=cmap, alpha=0.7)
        im.set_array(masked_vel_3d_mm.ravel())
...
def plotValidationPredictionScatter_4x2(validation_data, project_dir, outfolder):
    ...
    #ortho data
    ortho_ncc = pd.read_csv(project_dir / "1_Data" / "3_CIAS" / "results_validation" / "validation_ncc_ortho_image.csv")
    ortho_of = pd.read_csv(project_dir / "2_Optical_Flow_Results" / "validation"/ "Validation_Subset_ortho.csv")

    #hs data
    hs_ncc = pd.read_csv(project_dir / "1_Data" / "3_CIAS" / "results_validation" / "validation_ncc_hillshade_azi315_ang45.csv")
    hs_of = pd.read_csv(project_dir / "2_Optical_Flow_Results" / "validation"/ "Validation_Subset_hs_315_45.csv")
    ...
    hs24_ncc.loc[hs24_ncc.dx == -9999, 'HillshadeFilter'] = 1
    ...
    hs24_of.loc[hs24_of_filt.X_2020_PRED.isnull(), 'HillshadeFilter'] = 1
    
    hs168_of['HillshadeFilter'] = 0
    hs168_of.loc[hs168_of_filt.X_2020_PRED.isnull(), "HillshadeFilter"] = 1
    ...
    ## remove very large validation point...
    ortho_of = ortho_of[validation_dataset['magnitude']<150]
    ortho_ncc = ortho_ncc[validation_dataset['magnitude']<150]
    hs_of = hs_of[validation_dataset['magnitude']<150]
    hs_ncc = hs_ncc[validation_dataset['magnitude']<150]
    hs24_of = hs24_of[validation_dataset['magnitude']<150]
    hs24_ncc = hs24_ncc[validation_dataset['magnitude']<150]
    hs168_of = hs168_of[validation_dataset['magnitude']<150]
    validation_dataset = validation_dataset[validation_dataset['magnitude']<150]
    ...
    axes[0][0].scatter(x=validation_dataset['magnitude'], y=ortho_of['length'], c='#1E88E5', marker='.', alpha=0.5)
    axes[0][0].set_title("A.1 - Optical Flow, Ortho", fontsize = 10)
    ...
    axes[0][1].scatter(x=validation_dataset['magnitude'], y=ortho_ncc['length'], c=ortho_ncc['CorrelationFilter'], alpha=0.5, cmap = ListedColormap(['#1E88E5', '#D81B60']), marker='.')
    axes[0][1].set_title("A.2-3 - NCC, Ortho", fontsize = 10)
    axes[0][1].xaxis.set_visible(False)
    axes[0][1].yaxis.set_visible(False)
    plotyeqxline(axes[0][1])
    
    axes[1][0].scatter(x=validation_dataset['magnitude'], y=hs_of['length'], c='#1E88E5', alpha=0.5, marker='.')
    axes[1][0].set_title("B.1 - Optical Flow, Hillshade Pair", fontsize = 10)
    axes[1][0].xaxis.set_visible(False)
    axes[1][0].yaxis.set_visible(False)
    plotyeqxline(axes[1][0])
    
    axes[1][1].scatter(x=validation_dataset['magnitude'], y=hs_ncc['length'], c=hs_ncc['CorrelationFilter'], alpha=0.5, marker='.',cmap = ListedColormap(['#1E88E5', '#D81B60']))
    axes[1][1].set_title("B.2-3 - NCC, Hillshade Pair", fontsize = 10)
    axes[1][1].xaxis.set_visible(False)
    axes[1][1].yaxis.set_visible(False)
    plotyeqxline(axes[1][1])
    
    axes[2][0].scatter(x=validation_dataset['magnitude'], y=hs24_of['length'], c=hs24_of['HillshadeFilter'], alpha=0.5, marker='.',cmap = ListedColormap(['#1E88E5', "#FFC107"]))
    ...
    axes[2][1].scatter(x=validation_dataset['magnitude'], y=hs24_ncc['length'], c=hs24_ncc['Filter'], alpha=0.5, marker='.', cmap = cmap)
    ...
    axes[3][0].scatter(x=validation_dataset['magnitude'], y=hs168_of['length'], c=hs168_of['HillshadeFilter'], marker='.', alpha=0.5, cmap = ListedColormap(['#1E88E5', "#FFC107"]))
...
def plotErrorBandCorrCoeffDensity(file, outfolder):
    ...
    ax[1].set_xlabel(r"$log(\varepsilon)$")
